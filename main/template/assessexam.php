...
function myFunction() {
 ...
 <h4 class="panel-title">Assessment and Exam Marks for <em><?php 
...
$selectSQL = "SELECT * ,(final * 40 / 40) as pfinal , (full_mark * 100 / 100 ) as full ,((bonus_mark) * 60 / 60) as total FROM student,student_all_marks WHERE (student_all_marks.student_id=student.student_id) AND (semester_id='".$_SESSION['semester']."') AND (section='".$_SESSION['section']."') AND (course_id='".$_SESSION['course']."')"; 
...
<table id="basicTable" class="table table-striped table-bordered responsive">
    ...
    <?php
      ...
      if( mysql_num_rows( $selectRes )==0 ){
        ...
        echo "<tr><td>{$row['student_name']}</td><td>{$row['student_id']}</td><td>{$row['mid1']}</td><td>{$row['mid2']}</td><td>{$row['lab_mark']}</td><td>{$row['bonus']}</td><td>{$row['total']} %</td><td>{$row['final']}</td><td>{$row['full_mark']}</td><td>{$row['grade']}</td></tr>";
