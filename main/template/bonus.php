...
<html lang="en">
  ...
  $updatesql = "UPDATE `student_all_marks` SET `bonus`=('".$_POST['mrks'][$key]."') WHERE (student_mark_id= ".$_POST['bonusm'][$key].")";
  $result3 = mysql_query($updatesql) or die("<br/><br/>".mysql_error());
     ...
     echo '<tr><td colspan="4">Bonus Mark Should be Less or equals 3</td></tr>';
             }
    ...
    <?php
      ...
      if( mysql_num_rows( $selectRes )==0 ){
        ...
        while( $row = mysql_fetch_assoc( $selectRes ) ){
          echo "<tr><td>{$row['student_mark_id']}<input name='bonusm[]' type='hidden' value='{$row['student_mark_id']}'/></td><td>{$row['student_name']}</td><td>{$row['student_id']}<input name='std[]' type='hidden' value='{$row['student_id']}'/></td><td  contenteditable><input class='width100p' required name='mrks[]' value='{$row['bonus']}'/></td></tr>";
