                ...
                <div class="mainpanel">
                    <div class="pageheader">
                        <div class="media">
                            ...
                            <div class="media-body">
                                ...
                                <h4>Calculate Total Final Marks</h4>
                            </div>
                        </div><!-- media -->
                    </div><!-- pageheader -->
                   ...
                   <div class="panel panel-primary-head">
                            <div class="panel-heading">
                                <h4 class="panel-title">Calculate Total Final Marks</h4>
...
if($_POST['full_m'][$key] <= 100){
$_POST['gr'][$key] = "Excellent";
}
if($_POST['full_m'][$key] < 90){            
$_POST['gr'][$key] = "V.Good";
 }
if($_POST['full_m'][$key] < 80){            
 $_POST['gr'][$key] = "Good";
 }
if($_POST['full_m'][$key] < 70){            
 $_POST['gr'][$key] = "Pass";
 }
if($_POST['full_m'][$key] < 60){            
$_POST['gr'][$key] = "Fail";
 }

       			if($_POST['bonus_m'][$key] <=60){
	$updatesql = "UPDATE `student_all_marks` SET `full_mark`=('".$_POST['full_m'][$key]."'),`bonus_mark`=('".$_POST['bonus_m'][$key]."'),`grade`=('".$_POST['gr'][$key]."')  WHERE (student_mark_id= ".$_POST['ass'][$key].")";
	$result3 = mysql_query($updatesql) or die("<br/><br/>".mysql_error());
	
	 	  			}
            else {
$updatesql = "UPDATE `student_all_marks` SET `full_mark`=('".$_POST['full_m'][$key]."'),`bonus_mark`=60 ,`grade`=('".$_POST['gr'][$key]."')  WHERE (student_mark_id= ".$_POST['ass'][$key].")";
                 $result3 = mysql_query($updatesql) or die("<br/><br/>".mysql_error());
            }
 ...
 # Prepare the SELECT Query
  ...
  $selectSQL = "SELECT * , sum(assess_mark + mid_mark + bonus) as MTotal,  sum(bonus_mark + final) as Total FROM student,student_all_marks WHERE (student_all_marks.student_id=student.student_id) AND (semester_id='".$_SESSION['sem']."') AND (section='".$_SESSION['sec']."') AND (course_id='".$_SESSION['cou']."') group by student_all_marks.student_id ";
...
<table id="basicTable" class="table table-striped table-bordered responsive">
                                <thead class="">
                                    <tr>
                                        ...
                                        <th>Mid Exam Mark</th>
                                        <th>Bonus Mark</th>
                                        <th>Mark plus Bonus</th>
                                        <th>Final Exam Mark</th>
                                        <th>Total Final Mark</th>
                                        <th>Grade</th>
...
echo "<tr><td >{$row['student_mark_id']}<input name='ass[]' type='hidden' value='{$row['student_mark_id']}'/></td><td>{$row['student_name']}</td><td>{$row['student_id']}<input name='std[]' type='hidden' value='{$row['student_id']}'/></td><td>{$row['assess_mark']}<input name='ass_m[]' type='hidden' value='{$row['assess_mark']}'/></td><td>{$row['mid_mark']}<input name='mid_m[]' type='hidden' value='{$row['mid_mark']}'/></td><td>{$row['bonus']}</td><td><input  class='width100p' required name='bonus_m[]' type='hidden' value='{$row[MTotal]}'/>{$row[bonus_mark]}</td><td>{$row['final']}<input name='final_m[]' type='hidden' value='{$row['final']}'/></td><td><input  class='width100p' required name='full_m[]' type='hidden' value='{$row[Total]}'/>{$row[Total]}</td><td>{$row['grade']}<input name='gr[]' type='hidden' value='{$row['grade']}'/></td></tr>";
...
?>
                       ...
                       <div class="col-sm-9 col-sm-offset-3" align="right">
                         <button type="submit" class="btn btn-primary mr5" name="saveMark">Save</button>
                                            </div>
                                        </div>
                                    </div><!-- panel-footer -->
