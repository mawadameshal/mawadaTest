...
<html lang="en">
	...
	$updatesql = "UPDATE `student_all_marks` SET `mid1`=('".$_POST['mrks'][$key]."') WHERE (student_mark_id= ".$_POST['midexam'][$key].")";
    ...
    <?php
      ...
      if( mysql_num_rows( $selectRes )==0 ){
        ...
        while( $row = mysql_fetch_assoc( $selectRes ) ){
          echo "<tr><td>{$row['student_mark_id']}<input name='midexam[]' type='hidden' value='{$row['student_mark_id']}'/></td><td>{$row['student_name']}</td><td>{$row['student_id']}<input name='std[]' type='hidden' value='{$row['student_id']}'/></td><td  contenteditable><input class='width100p' required name='mrks[]' value='{$row['mid1']}'/></td></tr>";
