...
$result1 = mysql_query("SELECT `course_name` FROM `course` where `course_id`=".$_SESSION['course']);

$row2 = mysql_fetch_array($result1);

$_SESSION['coursen'] = $row2[0];
...
function myFunction() {
 ...
 <h4 class="panel-title" style="float:left; ">Marks Audit for <b><?php 
echo $_SESSION['coursen'];
?> </b>
 Course with <b><?php 
echo $userRow['username'];?></b> Lecturer</h4>
<span style="float: right;"> <?php echo "Date: " .date(" Y-m-d  h:i");?> </span>
                            ...
                            <table id="basicTable " class="table table-striped table-bordered responsive ">
                                <thead class="panel-heading" style="background-color:gray">
                                    <tr>
                                        <th>CRN</th>
                                        <th>Section No.</th>
                                        <th>Code</th>
                               ...
                               <td><?php echo $_SESSION['coursen'];?></td>
...
$selectSQL = "SELECT * , sum(assess_mark + mid_mark) as work FROM student,student_all_marks WHERE (student_all_marks.student_id=student.student_id) AND (semester_id='".$_SESSION['semester']."') AND (section='".$_SESSION['section']."') AND (course_id='".$_SESSION['course']."') group by student_all_marks.student_id"; 
...
<table id="basicTable" class="table table-striped table-bordered responsive">
                                <thead class="panel-heading" style="background-color:gray">
                                    <tr>
                                        ...
                                        <th>Student Name</th>
                                        <th>Course Case</th>
                                        <th>Semester working</th>
                                        <th>Final Exam</th>
                                        <th>Entered Mark</th>
                                        <th>Currently sending mark</th>
                                        <th>Sending mark</th>
                                             </tr>
                                </thead>
    ...
    <?php
      ...
      if( mysql_num_rows( $selectRes )==0 ){
        ...
        echo "<tr><td>".++$i."</td><td>{$row['student_id']}</td><td>{$row['student_name']}</td><td>{$row['case']}</td><td>{$row['bonus_mark']}</td><td>{$row['final']}</td><td>{$row['full_mark']}</td><td>*</td><td>*</td></tr>";
