...
?>
                ...
                <div class="mainpanel">
                    <div class="pageheader">
                        <div class="media">
                            ...
                            <div class="media-body">
                                ...
                                <h4>Calculate Assessment Marks</h4>
                            </div>
                        </div><!-- media -->
                    </div><!-- pageheader -->
                   ...
                   <div class="panel panel-primary-head">
                            <div class="panel-heading">
                                <h4 class="panel-title">Calculate Assessment Marks</h4>
	...
	$updatesql = "UPDATE `student_all_marks` SET `lab_mark`=('".$_POST['labmark'][$key]."'),`quiz_mark`=('".$_POST['quizmark'][$key]."'), `assess_mark`=('".$_POST['asesssmark'][$key]."') WHERE (student_mark_id= ".$_POST['ass'][$key].")";
 ...
 # Prepare the SELECT Query
  ...
  $selectSQL = "SELECT * ,  sum(lab1 + lab2) as labTotal, sum(quiz1 + quiz2) as quizTotal, sum(lab1 + lab2 + quiz1 + quiz2) as assessTotal FROM student,student_all_marks WHERE (student_all_marks.student_id=student.student_id) AND (semester_id='".$_SESSION['sem']."') AND (section='".$_SESSION['sec']."') AND (course_id='".$_SESSION['cou']."') group by student_all_marks.student_id ";
...
echo "<tr><td >{$row['student_mark_id']}<input name='ass[]' type='hidden' value='{$row['student_mark_id']}'/></td><td>{$row['student_name']}</td><td>{$row['student_id']}<input name='std[]' type='hidden' value='{$row['student_id']}'/></td><td>{$row['lab1']}<input name='labmark1[]' type='hidden' value='{$row['lab1']}'/></td><td>{$row['lab2']}<input name='labmark2[]' type='hidden' value='{$row['lab2']}'/></td><td><input  class='width100p' required name='labmark[]' type='hidden' value='{$row[labTotal]}'/>{$row[labTotal]}</td><td>{$row['quiz1']}<input name='quizmark1[]' type='hidden' value='{$row['quiz1']}'/></td><td>{$row['quiz2']}<input name='quizmark2[]' type='hidden' value='{$row['quiz2']}'/></td><td><input  class='width100p' required name='quizmark[]' type='hidden' value='{$row[quizTotal]}'/>$row[quizTotal]</td><td><input  class='width100p' required name='asesssmark[]' type='hidden' value='{$row[assessTotal]}'/>$row[assessTotal]</td> </tr>";
